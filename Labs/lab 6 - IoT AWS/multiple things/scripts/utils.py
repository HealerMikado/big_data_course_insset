...
def aws_iot_core_create_policy(detail=True):
    ...
    logger.info(f"\tStep 0: Checking policies ...")
...
def aws_iot_core_create_bulk_things():
    ...
    """
    logger.info(f"Registering bulky things ...")
    ...
    logger.info(f"\tChecking thingType")
    thingType_name = THING_TYPE_NAME
    ...
    if(thingType_name in thingTypes["thingTypeNames"]):
        ...
        iot_client.create_thing_type(thingTypeName=thingType_name, thingTypeProperties={
...
def aws_s3_reset():
    ...
    for bucket_name in bucket_names:
        ...
        for key in bucket.objects.all():
            key.delete()
            logger.info(f"Deleting the key {key}")

        bucket.delete()
        logger.info(f"Deleting the bucket {bucket_name}")
...
def aws_iot_core_get_all_policies(detail=False):
    ...
    # Log Info
    if(detail):
        logger.info("aws-iot-core: Getting policies ...")
...
def aws_iot_core_get_all_thing_types(detail=False):
    ...
    # Log Info
    if(detail):
        logger.info("Getting thing types")
...
def aws_iot_core_delete_all_policies():
    ...
    if(policies_count == 0):
        ...
        logger.info(f"\t\t{policies_count} policies are found.")

    # Step : Deleting policies
    logger.info("\tStep 2: Deleting policies ...")
    for policyName in policies["policyNames"]:
        iot_client.delete_policy(policyName=policyName)
        logger.info(f"\t\tDeleting the policyName: {policyName}")
...
def aws_iot_core_create_certificates():
    ...
    if(SET_CERT_UNIQUE):
        for thing in things['thingNames']:
            ...
            for file in glob.glob(f"{PATH_TO_THING_CERTIFICATE}/{thing}/*"):
                logger.info(f"\t\tDeleting the file {file}")
                os.remove(file)
            
            Path(f"{PATH_TO_THING_CERTIFICATE}/{thing}").mkdir(parents=True, exist_ok=True)


            # Storing the private key
            f = open(f"{PATH_TO_THING_CERTIFICATE}/{thing}/private.pem.key", "w")
            f.write(key_private)
            f.close()

            # Storing the public key
            f = open(f"{PATH_TO_THING_CERTIFICATE}/{thing}/public.pem.key", "w")
            f.write(key_public)
            f.close()

            # Storing the certificate
            f = open(f"{PATH_TO_THING_CERTIFICATE}/{thing}/certificate.pem.crt", "w")
            f.write(certificate)
            f.close()

            # Copy CA certificate
            shutil.copy2(PATH_TO_ROOT_CA, f'{PATH_TO_THING_CERTIFICATE}/{thing}/AmazonRootCA1.pem') # complete target filename given
    else:
        # Create keys and certificates
        response = iot_client.create_keys_and_certificate(setAsActive=True)
        ...
        # Storing the certificate
        f = open(f"{PATH_TO_THING_CERTIFICATE}/{thing}/certificate.pem.key", "w")
        f.write(certificate)
        f.close()
...
def aws_iot_core_delete_all_certificates(detail=True):
    ...
    # Log Info
    if(detail):
        logger.info(f"Deleting certificates")

    # Step 0: Get the certificates
    logger.info(f"\tStep 0: Getting certificates ...")
    ...
    logger.info(f"\tStep 2: Deleting certificates...")
    ...
    logger.info(f"\t\tDeleting certificates is completed.")
...
def aws_iot_core_delete_all_things(detail=True):
    ...
    # Log Info
    if(detail):
        logger.info(f"Deleting things")

    # Step 0: Get the things
    logger.info(f"\tStep 0: Getting things ...")
    things = aws_iot_core_get_all_things(detail=False)
    ...
    if(things_count == 0):
        ...
        logger.info(f"\t\t{things_count} things are found.")
    ...
    for thingName in things["thingNames"]:
        ...
        logger.info(f"\t\tDeleting thingName: {thingName}")
...
def aws_s3_config():
    ...
    # Log info
    logger.info("Configuring the S3 Bucket")
    ...
    logger.info(f"\tProvision file is succesfully uploaded")
...
def aws_iot_core_attach_certificates(detail=True):
    ...
    # Log info
    logger.info("Attaching certificates and things ")
